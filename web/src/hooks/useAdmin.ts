...
// ── Legacy react-query hooks (main's surface; colliding names suffixed Rq) ────

import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { useSearchParams } from 'react-router-dom';
import { adminApi } from '@/api/admin';

export function usePoliciesRq() {
  return useQuery({
    queryKey: ['policies'],
...
export function useDeletePolicyRq() {
  const qc = useQueryClient();
  return useMutation({
...
export function useApprovalsRq() {
...
export function useDecideApprovalRq() {
  const qc = useQueryClient();
  return useMutation({
