...
mod tests {
    ...
    async fn unknown_lookup_returns_empty() {
        ...
        let results = registry.resolve_all(&["{{ lookup:unknown(identity.id) }}"], &ctx()).await;
        assert_eq!(results.get("unknown(user-42)").map(String::as_str), Some(""));
    }
    ...
    async fn custom_handler_resolves() {
        ...
        let results = registry.resolve_all(&["{{ lookup:greet(identity.id) }}"], &ctx()).await;
        assert_eq!(
