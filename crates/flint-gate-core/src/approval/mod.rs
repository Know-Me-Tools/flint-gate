...
/// Durable approval-store backends (`memory` / `postgres`) behind an async
/// persistence trait. Distinct from the in-process [`ApprovalManager`] routing
/// table above: this family persists approval records across restarts and
/// replicas. Selected via `approval.backend` in config.
pub mod durable;
