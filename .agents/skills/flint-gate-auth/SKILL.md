...
description: Configure flint-gate auth providers (Kratos session, JWT/JWKS, API keys, anonymous) and rotate outbound JWT signing keys. Use when the user says "setup auth flint gate", "rotate jwt key", or is wiring an identity provider to a route.
---
